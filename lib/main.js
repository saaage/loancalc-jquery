...
$(function(){
  /* on page load run following script*/
    ...
    /*add an event listener to $submitButton*/
      ...
      /*Use checkValueYears/checkValueMonths to assign a value to magicNumber*/
      if (checkValueYears()) {
        magicNumber = Math.pow((1+rate), years*12);
      }else if (checkValueMonths()) {
        magicNumber = Math.pow((1+rate), months);
      }

      /*payment is calculated*/
      let payment = (principal*((rate*magicNumber)/(magicNumber-1))).toFixed(2);
